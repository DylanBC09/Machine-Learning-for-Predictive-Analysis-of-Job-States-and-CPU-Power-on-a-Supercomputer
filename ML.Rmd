...
data.clasificacion.num.norm <- scale(data.clasificacion.num, center = TRUE, scale = TRUE)
data.clasificacion.n <- data.frame(data.clasificacion.num.norm, data.clasificacion.cat)

Corrc(data.clasificacion)

data.clasificacion <- data.clasificacion %>% select(-ResvCPURAW, -SubmitWeekday)
...
# Tus gráficos
g1 <- Corrc(data.clasificacion)[[17]]
g2 <- Corrc(data.clasificacion)[[18]]
g3 <- Corrc(data.clasificacion)[[19]]
g4 <- Corrc(data.clasificacion)[[20]]
g5 <- Corrc(data.clasificacion)[[21]]
g6 <- Corrc(data.clasificacion)[[22]]
g7 <- Corrc(data.clasificacion)[[23]]
...
## Preselección de modelos: RandomForest y XGBoosting

balance categorías: 
table(taprendizaje$State) 
prop.table(table(taprendizaje$State))
...
Clase1 <- factor(ttesting$State)
library(traineR)
roc_curve <- ROC.plot(y_true = Clase1, y_prob = Estado1, positive = "COMPLETED")
plot(roc_curve, color = "red")
```
...
set.totaln <- as.data.frame(scale(set.totaln, center=T, scale=T)) 
...
modelo2 <- randomForest(CPUPower ~ ., ttraining, mtry = 4, ntree = 100, importance = TRUE)
predicciones <- predict(modelo2, ttesting)
